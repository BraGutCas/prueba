...
class SimuladorProcesos:
    def __init__(self, root):
        ...
        self.root.bind('<t>', lambda event: self.mostrar_tabla())
    ...
    def agregar_proceso(self):
        ...
        if len(self.procesos) == 4 and self.proceso_actual:
            self.procesos_espera.append(proceso)   
        elif len(self.procesos) < 5:
            proceso['tiempo_llegada'] = 0
            self.procesos.append(proceso)  # Agregar a la lista de procesos pendientes
        else:
            self.procesos_espera.append(proceso)  

        # Incrementar el ID para el siguiente proceso
    ...
    def actualizar_tablas(self):
        ...
        for proceso in self.procesos[:4]:  # Mostrar solo los primeros 4 procesos en pendientes
    ...
    def actualizar_contadores(self):
        if self.total_start_time and not self.simulador_finalizado and not self.paused:
            ...
            # Actualizar el tiempo bloqueado de cada proceso bloqueado si el simulador no está pausado
    ...
    def procesar_proceso(self):
        try:
            if self.procesos[-1]['tiempo_llegada'] == None:
                self.procesos[-1]['tiempo_llegada'] = int(time.time() - self.total_start_time) + self.total_elapsed_time - self.total_pause_time
        except:
            print()
    ...
    def mostrar_tabla(self):
        ...
        if self.proceso_actual:
            ...
            tiempo_transcurrido = int(time.time() - self.start_time)
            contador = int(time.time() - self.total_start_time)
            self.contador_antiguo += contador
            tiempo_espera = self.contador_antiguo - self.proceso_actual['tiempo_llegada'] - tiempo_transcurrido
            restante_cpu = max(0, self.proceso_actual['tiempo_estimado'] - tiempo_transcurrido)
            ...
            text_tabla.insert(tk.END, f"{self.proceso_actual['id_proceso']}\t|{estado}|\t|{self.proceso_actual['a']} {self.proceso_actual['operacion']} {self.proceso_actual['b']}|\t|{resultado}|\t|{self.proceso_actual['tiempo_llegada']}|\t|N/A|\t|N/A|\t|{tiempo_espera}|\t|{self.proceso_actual['tiempo_estimado']}|\t|{restante_cpu}|\t|{tiempo_respuesta}|\n")
        ...
        for proceso in self.procesos:
            ...
            tiempo_transcurrido = int(time.time() - self.start_time)
            tiempo_espera = self.contador_antiguo - proceso['tiempo_llegada'] - proceso['tiempo_transcurrido']
            ...
            text_tabla.insert(tk.END, f"{proceso['id_proceso']}\t{estado}\t{proceso['a']} {proceso['operacion']} {proceso['b']}\t{resultado}\t{proceso['tiempo_llegada']}\tN/A\tN/A\t|{tiempo_espera}|\t{proceso['tiempo_estimado']}\t{restante_cpu}\t{tiempo_respuesta}\n")
        ...
        for proceso in self.procesos_bloqueados:
            ...
            tiempo_transcurrido = int(time.time() - self.start_time)
            tiempo_espera = self.contador_antiguo - proceso['tiempo_llegada'] - proceso['tiempo_transcurrido']
            print(self.proceso_actual['id_proceso'],self.proceso_actual['tiempo_llegada'],proceso['tiempo_transcurrido'],self.contador_antiguo)
            restante_cpu = max(0, proceso['tiempo_estimado'] - proceso['tiempo_transcurrido'])
            tiempo_respuesta = proceso['tiempo_inicio'] - proceso['tiempo_llegada'] if proceso['tiempo_inicio'] is not None else "N/A"
            text_tabla.insert(tk.END, f"{proceso['id_proceso']}\t{estado}\t{proceso['a']} {proceso['operacion']} {proceso['b']}\t{resultado}\t{proceso['tiempo_llegada']}\tN/A\tN/A\t{tiempo_espera}\t{proceso['tiempo_estimado']}\t{restante_cpu}\t{tiempo_respuesta}\n")
        ...
        for proceso in self.procesos_espera:
            ...
            restante_cpu = max(0, proceso['tiempo_estimado'] - proceso['tiempo_transcurrido'])
            tiempo_respuesta = proceso['tiempo_inicio'] - proceso['tiempo_llegada'] if proceso['tiempo_inicio'] is not None else "N/A"
            text_tabla.insert(tk.END, f"{proceso['id_proceso']}\t{estado}\t{proceso['a']} {proceso['operacion']} {proceso['b']}\t{resultado}\t{proceso['tiempo_llegada']}\tN/A\tN/A\tN/A\t{proceso['tiempo_estimado']}\t{restante_cpu}\t{tiempo_respuesta}\n")
    ...
    def nuevo_proceso(self):
        ...
        if len(self.procesos) <= 4 and not self.procesos_espera:
