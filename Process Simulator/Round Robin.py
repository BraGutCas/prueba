...
class SimuladorProcesos:
    ...
    def agregar_proceso(self, tecla_n_presionada=False):
        operaciones = ["+", '-', '*', '/', '%']
        proceso = {
        ...
        self.total_procesos_activos = len(self.procesos) + len(self.procesos_bloqueados) + (1 if self.proceso_actual else 0)
        print('Total procesos activos: ',self.total_procesos_activos)

        if self.tecla_n_presionada and self.total_procesos_activos < 5:
            # Si se presionó 'n' y hay menos de 5 procesos activos, se agrega a la lista de procesos.
            proceso['tiempo_llegada'] = int(time.time() - self.total_start_time) + self.total_elapsed_time - self.total_pause_time
            self.procesos.append(proceso)
        else:
            if self.total_procesos_activos == 5 and self.proceso_actual:
                # Si hay exactamente 5 procesos activos, el nuevo proceso debe ir a la lista de espera.
                proceso['tiempo_llegada'] = int(time.time() - self.total_start_time) + self.total_elapsed_time - self.total_pause_time
                self.procesos_espera.append(proceso)
            elif self.total_procesos_activos < 5 and self.total_start_time is None:
                # Si hay menos de 5 procesos activos y no se ha iniciado el tiempo, agregarlo directamente a procesos.
                proceso['tiempo_llegada'] = 0
                self.procesos.append(proceso)
            elif self.total_procesos_activos == 5 and self.total_start_time is None:
                # Si hay menos de 5 procesos activos y no se ha iniciado el tiempo, agregarlo directamente a procesos.
                ...
                # En cualquier otro caso, el proceso va a la lista de espera.
                self.procesos_espera.append(proceso)
    ...
    def actualizar_contadores(self):
        if self.total_start_time and not self.simulador_finalizado and not self.paused:
            ...
            if self.proceso_actual:
                ...
                print('Tiempo transcurrido Quantum: ',self.proceso_actual['tiempo_transcurrido_quantum'])
